...
void difference(Mat& dil, Mat ero){
...
Mat hopeErode(Mat m, int element_size){
  ...
  imshow("ERODED FUNZIONA ZIO PERA", eroded*255);
  return eroded;
}
...
Mat hopeDilate(Mat m, int element_size) {
  ...
  while (n < element_size -1) {
    ...
    imshow("DILATION FUNZIONA ZIO PERA", dilated*255);
...
int main(int argc, char* argv[])
{
  ...
  imshow("CORRECT dilation", dilation_dst*255);
  ...
  difference(dilation_dst, erosion_dst);

  solution = dilation_dst.clone();
  printf("DIFFERENCE COMPLETED SUCCESSFUL\n");
